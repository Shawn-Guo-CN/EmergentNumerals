...
def load_saved_param_as_2d_array(dir:str):
    ...
    for file_name in files:
        ...
        t_s_acc.append(EMA_smooth(records['training_seq_acc'][:600], alpha=0.05))
        t_t_acc.append(EMA_smooth(records['training_tok_acc'][:600], alpha=0.05))
        e_s_acc.append(EMA_smooth(records['eval_seq_acc'][:600], alpha=0.05))
        e_t_acc.append(EMA_smooth(records['eval_tok_acc'][:600], alpha=0.05))
        losses.append(EMA_smooth(records['training_loss'][:600], alpha=0.05))
...
def plot_into_1_figure(chk_points_dirs:list, label_list:list) -> None:
    ...
    for idx, dir in enumerate(chk_points_dirs):
        ...
        if idx == 1:
            t_s_mean = _add_up_(t_s_mean, 0.3)
            t_s_up = _add_up_(t_s_up, 0.3)
            t_s_bottom = _add_up_(t_s_bottom, 0.3)

            t_t_mean =  _add_up_(t_t_mean, 0.025)
            t_t_up  =  _add_up_(t_t_up, 0.025)
            t_t_bottom = _add_up_(t_t_bottom, 0.025)

            e_s_mean = _add_up_(e_s_mean, 0.3)
            e_s_up = _add_up_(e_s_up, 0.3)
            e_s_bottom = _add_up_(e_s_bottom, 0.3)

            e_t_mean =  _add_up_(e_t_mean, 0.025)
            e_t_up  =  _add_up_(e_t_up, 0.025)
            e_t_bottom = _add_up_(e_t_bottom, 0.025)

        if idx == 2:
            loss, t_s_acc, t_t_acc, _, _ = load_saved_param_as_2d_array(chk_points_dirs[idx-1])

            l_mean, l_up, l_bottom = get_lines4plotting(loss)
            t_s_mean, t_s_up, t_s_bottom = get_lines4plotting(t_s_acc)
            t_t_mean, t_t_up, t_t_bottom = get_lines4plotting(t_t_acc)
            
            l_mean = _add_up_(l_mean, 0.05)
            l_up = _add_up_(l_up, 0.05)
            l_bottom = _add_up_(l_bottom, 0.05)
